...
namespace Apparel_Dynamic_1._0.Resources.Setup
{
    ...
    class ProductLine : UserFormBase
    {
            ...
            try
            {
                ...
                if (oForm.Mode == SAPbouiCOM.BoFormMode.fm_ADD_MODE || oForm.Mode == SAPbouiCOM.BoFormMode.fm_UPDATE_MODE)
                {
                    ...
                    if (!string.IsNullOrEmpty(UCode))
                    {
                        ...
                        string query = $@"SELECT 1 FROM ""@FIL_MH_OPLM"" WHERE ""Code"" = '{UCode.Replace("'", "''")}'";
                        oRS.DoQuery(query);
        ...
        {
            string code = oForm.DataSources.DBDataSources.Item("@FIL_MH_OPLM").GetValue("Code", 0);
            string name = oForm.DataSources.DBDataSources.Item("@FIL_MH_OPLM").GetValue("Name", 0);
            string gender = oForm.DataSources.DBDataSources.Item("@FIL_MH_OPLM").GetValue("U_GENDER", 0);
            if (code == "")
            {
                Global.GFunc.ShowError("Enter Product Line Mater Code");
                oForm.ActiveItem = "ETCODE";
                return BubbleEvent = false;
            }
            else if (name == "")
            {
                Global.GFunc.ShowError("Enter Product Line Master Name");
