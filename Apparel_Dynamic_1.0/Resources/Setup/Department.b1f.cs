...
{
    [FormAttribute("Apparel_Dynamic_1._0.Resources.Setup.Department", "Resources/Setup/Department.b1f")]
    class Department : UserFormBase
    {
        public Department()
        {
        }
            ...
            try
            {
                ...
                if (oForm.Mode == SAPbouiCOM.BoFormMode.fm_ADD_MODE || oForm.Mode == SAPbouiCOM.BoFormMode.fm_UPDATE_MODE)
                {
                    ...
                    if (!string.IsNullOrEmpty(UCode))
                    {
                        ...
                        string query = $@"SELECT 1 FROM ""@FIL_MH_ODEPT"" WHERE ""Code"" = '{UCode.Replace("'", "''")}'";
                        oRS.DoQuery(query);
        ...
        {
            string code = oForm.DataSources.DBDataSources.Item("@FIL_MH_ODEPT").GetValue("Code", 0);
            string name = oForm.DataSources.DBDataSources.Item("@FIL_MH_ODEPT").GetValue("Name", 0);
            if (code == "")
            {
                Global.GFunc.ShowError("Enter Department Code");
                oForm.ActiveItem = "ETCODE";
                return BubbleEvent = false;
            }
            else if (name == "")
            {
                Global.GFunc.ShowError("Enter Department Name");
