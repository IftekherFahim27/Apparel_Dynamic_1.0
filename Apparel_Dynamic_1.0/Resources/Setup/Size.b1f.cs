...
{
    [FormAttribute("Apparel_Dynamic_1._0.Resources.Setup.Size", "Resources/Setup/Size.b1f")]
    class Size : UserFormBase
    {
        public Size()
        {
        }
            ...
            try
            {
                ...
                if (oForm.Mode == SAPbouiCOM.BoFormMode.fm_ADD_MODE || oForm.Mode == SAPbouiCOM.BoFormMode.fm_UPDATE_MODE)
                {
                    ...
                    if (!string.IsNullOrEmpty(UCode))
                    {
                        ...
                        string query = $@"SELECT 1 FROM ""@FIL_MH_SIZEMSTR"" WHERE ""Code"" = '{UCode.Replace("'", "''")}'";
                        oRS.DoQuery(query);
        ...
        {
            string code = oForm.DataSources.DBDataSources.Item("@FIL_MH_SIZEMSTR").GetValue("Code", 0);
            string name = oForm.DataSources.DBDataSources.Item("@FIL_MH_SIZEMSTR").GetValue("Name", 0);
            if (code == "")
            {
            ...
            {
                Global.GFunc.ShowError("Enter Size Name");
