...
{
    [FormAttribute("Apparel_Dynamic_1._0.Resources.Setup.UseType", "Resources/Setup/UseType.b1f")]
    class UseType : UserFormBase
    {
        public UseType()
        {
        }
            ...
            try
            {
                ...
                if (oForm.Mode == SAPbouiCOM.BoFormMode.fm_ADD_MODE || oForm.Mode == SAPbouiCOM.BoFormMode.fm_UPDATE_MODE)
                {
                    ...
                    if (!string.IsNullOrEmpty(UCode))
                    {
                        ...
                        string query = $@"SELECT 1 FROM ""@FIL_MH_USETYPE"" WHERE ""Code"" = '{UCode.Replace("'", "''")}'";
                        oRS.DoQuery(query);
        ...
        {
            string code = oForm.DataSources.DBDataSources.Item("@FIL_MH_USETYPE").GetValue("Code", 0);
            string name = oForm.DataSources.DBDataSources.Item("@FIL_MH_USETYPE").GetValue("Name", 0);
            if (code == "")
            {
                Global.GFunc.ShowError("Enter Use Type Code");
                oForm.ActiveItem = "ETCODE";
                return BubbleEvent = false;
            }
            else if (name == "")
            {
                Global.GFunc.ShowError("Enter Use Type Name");
